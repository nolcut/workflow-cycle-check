...
faasr_check_workflow_cycle_original <- function(faasr){
  ...
  dfs <- function(start, target, stack){
    ...
    if (target %in% graph[[start]]) {
      message(paste("Elapsed time: ", as.numeric(Sys.time() - start_time)))
  ...
  message(paste("(Successful) elapsed time: ", as.numeric(Sys.time() - start_time)))
